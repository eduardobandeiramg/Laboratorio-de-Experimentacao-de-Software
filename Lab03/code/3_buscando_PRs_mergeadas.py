...
def buscandoDadosPRsComPaginacao(autor, nome, aPartirDe):
    ...
    repository(owner:"{autor}", name:"{nome}"){{
    pullRequests(states:MERGED, first:20, after:"{aPartirDe}"){{
    totalCount
    nodes{{
    number
    files(first:1){{
    ...
    for linha in planilha:
        ...
        while percorrePaginas:
            if resposta is not None and resposta.status_code == 200:
                ...
                totaRepos+=1
                break  
    with open("Lab03/planilhas/dados_pull_requests_mergeadas.csv", mode="w", encoding="utf-8", newline="") as arquivo2:
